...
def show_hint():
    """Display the original image as a hint for 5 seconds."""
    ...
    try:
        ...
        # Close existing hint figure if it exists
        if hint_fig is not None:
            plt.close(hint_fig)
            hint_fig = None

        # Create new hint figure
        hint_fig = plt.figure(figsize=(5, 5))
        plt.imshow(original_image)
        plt.title(f"Hint - Original Image ({hint_usage_count + 1}/{HINT_LIMIT})")
        plt.axis('off')
        
        # Make sure the figure is drawn and shown
        hint_fig.canvas.draw()
        hint_fig.canvas.flush_events()
        plt.show(block=False)
        
        hint_usage_count += 1
        logger.info(f"Hint displayed ({hint_usage_count}/{HINT_LIMIT})")
        
        # Schedule the hint to close after 5 seconds
        plt.pause(5)
        hide_hint()
...
def hide_hint():
    ...
    try:
        if hint_fig is not None:
            plt.close(hint_fig)
            hint_fig = None
            # Force a redraw of the main figure
            if fig is not None:
                fig.canvas.draw()
...
def on_key_press(event):
    """Handle keyboard press events for controls."""
    try:
        if event.key == 'q':
...
def on_key_release(event):
    """Handle keyboard release events for controls."""
    try:
        if event.key == 'h':  # Hide hint when 'h' is released
            hide_hint()
            # Force redraw of main figure
            if fig is not None:
                fig.canvas.draw()
...
def user_interaction():
    ...
    try:
        ...
        print(f"- Press 'h' to show hint for 5 seconds - {HINT_LIMIT} uses available")
...
def get_difficulty_level():
    ...
    while True:
        ...
        if choice == '1':
            while True:
                print("\nSelect Grid Size for Easy Level:")
                print("1. 2x2")
                print("2. 3x3")
                print("3. Back to Difficulty Selection")
                
                grid_choice = input("Enter your choice (1-3): ").strip()
...
if __name__ == "__main__":
    try:
        ...
        # Get difficulty level and grid size
        grid_size = get_difficulty_level()
